...
def get_resp(url):
    ...
    try:
        ...
        try:
            ...
            request = requests.get(url, headers=headers, proxies=get_proxies())
        return lh.fromstring(request.text)
    except Exception:
...
def write_part_file(args, url, raw_html, html=None, part_num=None):
    ...
    if raw_html:
        ...
        with open(filename, "w", encoding="utf-8", errors="ignore") as part:
...
def read_files(filenames):
    ...
    if isinstance(filenames, list):
        ...
        with open(filenames, "r", encoding="utf-8", errors="ignore") as infile:
            return infile.read()
